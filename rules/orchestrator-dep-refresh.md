...
Confirm the new dep version is actually installed in the running orchestrator before declaring the refresh complete. Substitute `DEP_NAME` with the package name (the verification command is shown with a placeholder rather than `<dep-name>` so it pastes cleanly — `<` would otherwise be parsed as shell input redirection):
