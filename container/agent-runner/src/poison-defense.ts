...
/**
 * Walks an MCP tool response and sanitizes each text block in place.
 *
 * Two response shapes are supported:
 *  - the standard wrapped form
 *    `{ content: [{ type: 'text', text: string }, ...] }`
 *  - the bare-array form `[{ type: 'text', text: string }, ...]` that
 *    some clients pass through unwrapped
 *
 * Without handling both, the walker silently misses the bare-array
 * form and the sanitizer becomes a no-op for those responses — the
 * exact invisible-Unicode smuggling vector this hook is supposed to
 * close.
 *
