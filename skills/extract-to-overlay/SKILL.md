...
The target tile is shipped when `Skill(skill: "release")` Step 7 reports the release confirmed — not on a green run, and not on a registry advance. Step 5's `update_group_config` validator requires the confirmed version. The source tile MUST NOT ship before the target: Step 6 is gated on this ordering.
...
The source tile is shipped on the same signal — `Skill(skill: "release")` Step 7 reporting the release confirmed. Do not proceed to Step 7 of this skill before then.
