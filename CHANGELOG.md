...
- **tessl-version-floating** (new — filename retained, rule body rewritten per OpenAI policy reviewer feedback on `nanoclaw-host#14`) — Pins in `tessl-workspace/tessl.json` are correct per `coding-policy: dependency-management`; the actual bug is `tessl update` rewriting the manifest in-place without committing the bump, so a `git pull` rolls the orchestrator back. Codifies that all three `tessl update` callers (`scripts/deploy.sh`, the 15-min orchestrator catch-up loop, the `tessl_update` MCP tool) MUST auto-commit + push `tessl-workspace/tessl.json` after a successful update with a `[skip ci]` `chore(tessl): bump tile pins from registry` commit. Reproducibility preserved (the pin in the manifest IS the lock); drift fixed (every bump reaches git). Triggered by NAS inspection on 2026-04-27 showing the working tree 22 days behind `main` across every tile (e.g. core 0.1.60 → 0.1.87, admin 0.1.101 → 0.1.229).
