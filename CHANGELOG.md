...
Only the unambiguous renames ride along: the `publish.yml` display name ("Review & Publish Tile" → "Review & Publish Plugin", matching every other migrated repo in the fleet), the plugin description, and `tessl tile lint` → `tessl plugin lint` in the `promote` and `extract-to-overlay` skills.
