...
| [tessl-version-floating](rules/tessl-version-floating.md) | Authority-of-record for the Runtime-Managed Manifest Carve-Out (approved exception to `coding-policy: dependency-management`). Every covered `tessl.json` declares `"mode": "managed"` and floats `jbaruch/*` at `latest`; third-party pins, except in `jbaruch/nanoclaw`'s two unattended-update manifests where everything floats. Enforcement: `deploy.sh` step 3b for that pair, `coding-policy`'s `check-tessl-latest.sh` `SessionStart` hook for the nine plugin repos. |
