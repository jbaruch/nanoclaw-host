...
| [sync-cli-floating](rules/sync-cli-floating.md) | The agent image's `reclaim-tripit-timezones-sync` install MUST stay unpinned (approved exception to `coding-policy: dependency-management`, First-Party Co-Shipped Dependency Carve-Out) — same operator owns both repos, so the pin protected nothing and silently held the image on a pre-OneCLI build. `deploy.sh` verifies the reference is bare AND that the `ADD` refetch trigger is present. |
